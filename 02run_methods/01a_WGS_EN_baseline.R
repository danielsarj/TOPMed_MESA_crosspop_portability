...
get_covariates <- function(covariate_file_name, samples) {
  ...
  cov_df
} ##no covariates, so this function is not used - see line 171
...
nested_cv_elastic_net_perf <- function(x, y, n_samples, n_train_test_folds, n_k_folds, alpha) {
  ...
  for (test_fold in 1:n_train_test_folds) {
    ...
    y_pred <- tryCatch({
      ...
      fit <- cv.glmnet(x_train, y_train, nfolds = n_k_folds, alpha = alpha, type.measure='mse', foldid = cv_fold_ids, parallel = TRUE)
...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
  ...
  covariates_df <- NA#get_covariates(covariates_file, samples)
  ...
  tiss_chr_summ_f <- prefix %&% '_chr' %&% chrom %&% '_tiss_chr_summary.txt'
    ...
    if (ncol(cis_gt) >= 2) {
      ...
      if (length(fit) > 0) {
        ...
        if (fit$nzero[best_lam_ind] > 0) {
          ...
          weighted_snps_info <- snp_annot %>% filter(rsid %in% weighted_snps) %>% select(rsid, varID, refAllele, effectAllele)
