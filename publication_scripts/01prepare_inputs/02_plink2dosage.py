...
if __name__ == "__main__":
  ...
  subprocess.call([
    ...
    '--recode', 'A-transpose', '--keep-allele-order', '--out', args.out
  ])

  # Now calculate the minor allele frequency:
  subprocess.call([
    args.plink, '--bfile', args.bfile, '--freq', '--keep-allele-order', '--out', args.out
  ])
  ...
  for curCHR, lines in buff.items():
    with open(args.out + curCHR + ".txt", "w") as ofile:
